...
void Node::create(std::vector<int> _values)
{
	...
	for (int i = 0; i < (int)values.size(); ++i) {
		valueTexts[i].setFont(*Resources::Font::arial);
		valueTexts[i].setString(Tools::String::toString(values[i]));
		valueTexts[i].setCharacterSize(Layout::DisplayComponent::Node::height / 2);
		...
		valueShapes[i].setSize({valueTexts[i].getGlobalBounds().width + 30, height - 2 * spacing});
		valueShapes[i].setFillColor(Color::GrayBG);
		valueShapes[i].setCornersRadius(valueShapes[i].getSize().y / 2);
...
void Node::update()
{
	...
	nodeShape.setSize({px, Layout::DisplayComponent::Node::height});
}
